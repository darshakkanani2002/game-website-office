...
"use strict"; {
    ...
    const p = /safari\//i.test(navigator.userAgent) && !/chrome\/|chromium\/|edg\//i.test(navigator.userAgent);
    let u = new Audio;
    const z = {
        "audio/webm; codecs=opus": !!u.canPlayType("audio/webm; codecs=opus") && !p,
    ...
    window.RuntimeInterface = class g {
        constructor(c) {
            ...
            !this._useWorker || "undefined" !==
                typeof OffscreenCanvas && navigator.userActivation && m() || (this._useWorker = !1);
            ...
            "html5" !== this._exportType && "playable-ad" !== this._exportType || !this._isFileProtocol ||
        ...
        IsiOSWebView() {
            ...
            return d && I(this._exportType) || navigator.standalone || /crios\/|fxios\/|edgios\//i.test(c)
        }
        async _Init(c) {
            ...
            if ("playable-ad" === this._exportType) {
                ...
                for (let l = 0, n = c.engineScripts.length; l < n; ++l) {
                    ...
                    this._localFileStrings.hasOwnProperty(k) ?
                        ...
                        } : this._localFileBlobs.hasOwnProperty(k) && (c.engineScripts[l] = URL.createObjectURL(this._localFileBlobs[k]))
                }
            }
            c.baseUrl ? this._baseUrl = c.baseUrl : (k = location.origin, this._baseUrl = ("null" === k ? "file:///" : k) + location.pathname, k = this._baseUrl.lastIndexOf("/"), -1 !== k && (this._baseUrl = this._baseUrl.substr(0, k + 1)));
        ...
        _GetWorkerURL(c) {
            c = this._workerScriptURLs.hasOwnProperty(c) ?
                this._workerScriptURLs[c] : c.endsWith("/workermain.js") && this._workerScriptURLs.hasOwnProperty("workermain.js") ? this._workerScriptURLs["workermain.js"] : "playable-ad" === this._exportType && this._localFileBlobs.hasOwnProperty(c.toLowerCase()) ? this._localFileBlobs[c.toLowerCase()] : c;
            c instanceof Blob && (c = URL.createObjectURL(c));
            return c
        }
        async CreateWorker(c, k, l) {
            if (c.startsWith("blob:")) return new Worker(c, l);
            if ("cordova" === this._exportType && this._isFileProtocol) return c = await this.CordovaFetchLocalFileAsArrayBuffer(l.isC3MainWorker ?
                c : this._scriptFolder + c), c = new Blob([c], {
        ...
        async _InitWorker(c, k) {
            ...
            if ("cordova" === this._exportType)
                for (let q = 0, t = c.projectScripts.length; q < t; ++q) {
                    const x = c.projectScripts[q],
                        E = x[0];
                    if (E === c.mainProjectScript ||
                        "scriptsInEvents.js" === E || E.endsWith("/scriptsInEvents.js")) x[1] = await this._MaybeGetCordovaScriptURL(E)
                }
        ...
        async _InitDOM(c, k) {
            ...
            this._domHandlers = C.map(q => new q(this));
            this._FindRuntimeDOMHandler();
            var l = c.engineScripts.map(q => "string" === typeof q ? (new URL(q,
                this._baseUrl)).toString() : q);
            ...
            for (let [q, t] of r)
                if (t || (t = q), q === n) try {
                    t = await this._MaybeGetCordovaScriptURL(t), await b(t), "preview" !== this._exportType || l[q] || this._ReportProjectMainScriptError(q, "main script did not run to completion")
                } catch (x) {
                    this._ReportProjectMainScriptError(q,
                        x)
                } else if ("scriptsInEvents.js" === q || q.endsWith("/scriptsInEvents.js")) t = await this._MaybeGetCordovaScriptURL(t), await b(t);
        ...
        PostToRuntimeComponent(c, k, l, n, r) {
        ...
        async _MaybeGetCordovaScriptURL(c) {
            return "cordova" === this._exportType && (c.startsWith("file:") || this._isFileProtocol && this.IsRelativeURL(c)) ? (c.startsWith(this._baseUrl) && (c = c.substr(this._baseUrl.length)), c = await this.CordovaFetchLocalFileAsArrayBuffer(c),
                c = new Blob([c], {
                    type: "application/javascript"
        ...
        _HasPermission(c) {
            ...
            return new Promise((l, n) => k.checkPermission(this._MapPermissionID(k, c), r => l(!!r.hasPermission), n))
        }
        _RequestPermission(c) {
            const k = this._GetPermissionAPI();
            return new Promise((l, n) => k.requestPermission(this._MapPermissionID(k, c), r => l(!!r.hasPermission), n))
        }
        async RequestPermissions(c) {
            if ("cordova" !== this.GetExportType() || this.IsiOSCordova()) return !0;
            for (const k of c)
                if (!await this._HasPermission(k) &&
                    !1 === await this._RequestPermission(k)) return !1;
            return !0
        }
        ...
        async _ConvertDataUrisToBlobs() {
            const c = [];
            for (const [k, l] of Object.entries(this._localFileBlobs)) c.push(this._ConvertDataUriToBlobs(k, l));
            await Promise.all(c)
        }
        async _ConvertDataUriToBlobs(c, k) {
            if ("object" ===
                typeof k) this._localFileBlobs[c] = new Blob([k.str], {
                type: k.type
        ...
        _BinaryStringToBlob(c, k) {
            ...
            let n = l >> 2,
                ...
                t, x;
            for (x = t = 0; t < n; ++t) q[t] = c.charCodeAt(x++) | c.charCodeAt(x++) <<
                8 | c.charCodeAt(x++) << 16 | c.charCodeAt(x++) << 24;
    ...
    d.AddDOMHandlerClass(class extends self.DOMHandler {
        constructor(f) {
            ...
            "undefined" !== typeof PointerEvent ? (window.addEventListener("pointerdown", y, {
                passive: !1
            }), c.addEventListener("pointerdown", y)) : c.addEventListener("touchstart", y);
        ...
        Attach() {
            ...
            "undefined" !== typeof PointerEvent ? (window.addEventListener("pointerdown", g => {
                ...
                "undefined" !== typeof window.onpointerrawupdate && self === self.top ? (this._pointerRawUpdateRateLimiter = new self.RateLimiter(() => this._DoSendPointerRawUpdate(), 5), this._pointerRawUpdateRateLimiter.SetCanRunImmediate(!0), window.addEventListener("pointerrawupdate", g => this._OnPointerRawUpdate(g))) : window.addEventListener("pointermove", g => this._OnPointerEvent("pointermove", g)), window.addEventListener("pointerup", g => this._OnPointerEvent("pointerup", g)), window.addEventListener("pointercancel", g => this._OnPointerEvent("pointercancel",
                    g))) : (window.addEventListener("mousedown", g => {
                this._HandlePointerDownFocus(g);
            ...
            }), window.addEventListener("touchmove", g => this._OnTouchEvent("pointermove", g)), window.addEventListener("touchend",
                g => this._OnTouchEvent("pointerup", g)), window.addEventListener("touchcancel", g => this._OnTouchEvent("pointercancel", g)));
        ...
        _ScheduleSimulatedResize(f, g, c) {
            ...
            this._simulatedResizeTimerId = setTimeout(() =>
                this._OnSimulatedResize(f, g, c), 48)
        }
        ...
        _TrySetTargetOrientation() {
            ...
            if (screen.orientation &&
                screen.orientation.lock) screen.orientation.lock(f).catch(g => console.warn("[Construct 3] Failed to lock orientation: ", g));
            else try {
                let g = !1;
        ...
        _OnPointerEvent(f, g) {
            ...
            "mouse" === g.pointerType && (c = "mousemove", "pointerdown" === f ? c = "mousedown" : "pointerup" === f && (c = "mouseup"), this._OnMouseEvent(c, g, h), this._mousePointerLastButtons = g.buttons)
        }
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    _OnNavigate(d) {
        ...
        if ("back" === a) navigator.app && navigator.app.backHistory ? navigator.app.backHistory() : window.history.back();
            ...
            const b = d.target;
            d = d.exportType;
            self.cordova && self.cordova.InAppBrowser ? self.cordova.InAppBrowser.open(a, "_system") : "preview" === d || "windows-webview2" === d ? window.open(a, "_blank") : this._isScirraArcade || (2 === b ? window.top.location = a : 1 === b ? window.parent.location = a : window.location = a)
    ...
    self.AudioDOMHandler = class extends self.DOMHandler {
        ...
        async _AddEffect(a) {
            ...
            if ("filter" === b) h = new self.C3AudioFilterFX(this, ...h);
                ...
                h._SetBufferInfo(a.bufferOriginalUrl, a.bufferUrl,
                    a.bufferType)
            ...
            else if ("analyser" === b) h = new self.C3AudioAnalyserFX(this,
                ...h);
...
self.C3AudioBuffer = class {
    ...
    static Create(d, a, b, e, h) {
        ...
        return !h || d.IsPlayMusicAsSound() || m ? new self.C3WebAudioBuffer(d, a, b, e, h, m) : new self.C3Html5AudioBuffer(d,
            a, b, e, h)
    }
    CreateInstance(d) {
    ...
    self.C3AudioInstance = class {
        ...
        SetPan(a, b, e, h, m, p) {
            this._isPannerEnabled && (this.SetPanXYA(a, b, e), a = self.AudioDOMHandler.ToDegrees, this._pannerNode.coneInnerAngle =
                a(h), this._pannerNode.coneOuterAngle = a(m), this._pannerNode.coneOuterGain = p)
        }
        SetPanXYA(a, b, e) {
...
"use strict"; {
    ...
    self.C3AudioConvolveFX = class extends d {
        ...
        SetParam(a, b, e, h) {
            switch (a) {
                case 0:
                    b = Math.max(Math.min(b / 100, 1), 0), this._params[1] = b, this.SetAudioParam(this._wetNode.gain, b, e, h), this.SetAudioParam(this._dryNode.gain,
                        1 - b, e, h)
            }
        }
    ...
    self.C3AudioRingModFX = class extends d {
        ...
        SetParam(a, b, e, h) {
            switch (a) {
                ...
                case 7:
                    this._params[0] =
                        b, this.SetAudioParam(this._oscNode.frequency, b, e, h)
            }
        }
    };
            ...
            switch (a) {
                case 0:
                    b = Math.max(Math.min(b /
                        100, 1), 0), this._params[4] = b, this.SetAudioParam(this._wetNode.gain, b, e, h), this.SetAudioParam(this._dryNode.gain, 1 - b, e, h)
            }
        }
    };
